...
class Base#MODELCLASS(object):
    
    def __init__(self, data=None, schema=None):
        """ Basic instance setup"""
        #collection name is plural by convention 
        self.collection_name = "#MODEL_COLLECTION"
        #modelname is singular by convention
     ...
     def load_schema(self):
        try:
            """Tries to find the according schema in migrations/schemas/#MODELCLASS.py 
            imports the schema and sets the according properties in this class/instance"""
            #from copow.migrations.schemas.#MODEL_SCHEMA_schema import #MODELNAME as schema
            #from copow.migrations.schemas.#MODEL_SCHEMA_schema import #MODELNAME_relations as relations
            schema = reload(copow.migrations.schemas.#MODEL_SCHEMA_schema)
            self.schema = schema.__dict__["#MODELNAME"]
            self.relations = schema.__dict__["#MODELNAME_relations"]
    ...
    def setup_properties(self):
        ...
        for column, attrs in self.schema.items():
            ...
            if powlib.schema_types.has_key(type):
                ...
                raise Exception("no or unknown type given in schema: #MODELNAME_schema.py")
